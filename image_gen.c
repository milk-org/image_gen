...
)
{
    ...
    data.image[ID].array.F[((int)(naxes[1] / 2))*naxes[0] + ((int)(
                               naxes[0] / 2))] = intensity_1;
    data.image[ID].array.F[((int)(naxes[1] / 2 + separation * cos(
                                      position_angle)))*naxes[0] + ((int)(naxes[0] / 2 + separation * sin(
                                              position_angle)))] = intensity_2;
        ...
        for(seg = 0; seg < NBseg; seg++)
        {
            ...
            for(ii = 0; ii < xsize; ii++)
                for(jj = 0; jj < ysize; jj++)
                {
                    ...
                    data.image[IDmask].array.F[jj * xsize + ii] += (1.0 + seg) *
...
)
{
    ...
    for(uint32_t jj = 0; jj < naxes[1]; jj++)
        for(uint32_t ii = 0; ii < naxes[0]; ii++)
        {
            if((fabs(jj - y_center) / height) < exp(-((ii - x_center) *
                                                    (ii - x_center) / width / width)))
            {
                data.image[ID].array.F[jj * naxes[0] + ii] = 1;
            }
        }
...
)
{
    ...
    for(uint32_t jj = 0; jj < naxes[1]; jj++)
        for(uint32_t ii = 0; ii < naxes[0]; ii++)
        {
            ...
            data.image[ID].array.F[jj * naxes[0] + ii] = step * ((long)(
                        theta / 2.0 / PI * NB_sectors));
        }
...
)
{
    ...
    for(uint32_t jj = 0; jj < naxes[1]; jj++)
        for(uint32_t ii = 0; ii < naxes[0]; ii++)
        {
            distsq = (ii - naxes[0] / 2) * (ii - naxes[0] / 2) + (jj - naxes[1] / 2) *
                     (jj - naxes[1] / 2);
...
)
{
    ...
    while(i < nb_star)
    {
        ...
        angle = 2 * PI * ran1();
        uint32_t ii = (uint32_t)(naxes[0] / 2 + (cluster_size * naxes[0] / 2) * dist *
                                 cos(angle));
        uint32_t jj = (uint32_t)(naxes[1] / 2 + (cluster_size * naxes[1] / 2) * dist *
                                 sin(angle));
...
)
{
    ...
    for(uint32_t ii = 0; ii < naxes[0]; ii++)
        for(uint32_t jj = 0; jj < naxes[1] / 2 + 1; jj++)
        {
            ...
            r = sqrt(aob * x * x + boa * y * y);
            data.image[ID].array.F[jj * naxes[0] + ii] += E_L0 * pow(10.0,
                    (-3.3307 * (pow((r / E_radius), 0.25) - 1.0)));
        }
    ...
    total = 2.0 * PI * S_L0 * S_radius * S_radius + 23.02 * E_L0 * E_radius *
            E_radius;
...
)
{
    ...
    for(uint32_t jj = ljj; jj < hjj; jj++)
        for(uint32_t ii = lii; ii < hii; ii++)
        {
            ...
            x = x;
            y = y / sqrt(1 - E * E);
            distsq = (x * x + y * y) / (naxes[0] * naxes[0] + naxes[1] * naxes[1]) /
                     galaxy_size / galaxy_size;
            data.image[ID].array.F[jj * naxes[0] + ii] = (double) peak * exp(
                        -concentration * distsq);
        }
...
)
{
    ...
    for(uint32_t jj = 0; jj < naxes[1]; jj++)
        for(uint32_t ii = 0; ii < naxes[0]; ii++)
        {
            data.image[ID].array.F[jj * naxes[0] + ii] = sqrt((f1 - ii) * (f1 - ii) +
                    (f2 - jj) * (f2 - jj));
        }
...
)
{
    ...
    for(uint32_t jj = 0; jj < naxes[1]; jj++)
        for(uint32_t ii = 0; ii < naxes[0]; ii++)
        {
            ...
            if(i != 0)
            {
                data.image[ID].array.F[jj * naxes[0] + ii] = valarr[i - 1] +
                        (valarr[i] - valarr[i - 1]) * (dist - distarr[i - 1]) /
...
)
{
    ...
    for(uint32_t ii = 0; ii < size; ii++)
        for(uint32_t jj = 0; jj < size; jj++)
        {
            ...
            {
                data.image[ID].array.F[jj * size + ii] = 0.5 * (cos(PI * (dist - a) /
                        (b - a)) + 1.0);
            }
        }
...
)
{
    ...
    for(uint32_t ii = 0; ii < size; ii++)
        for(uint32_t jj = 0; jj < size; jj++)
        {
            ...
            data.image[IDout].array.F[jj * size + ii] = data.image[IDin].array.F[jj0 *
                    sizex0 + ii0];
        }
